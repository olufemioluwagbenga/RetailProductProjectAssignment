...
namespace StockaProSSO.Core.Services
{
    ...
    {
        private AuthorizeService _clearanceService = new AuthorizeService(new StockaProContext());
        ...
        public async Task<Clients> GetClient(string clientId = null, int? clientKey = null)
        {
            var result = new Result<ClientModel>(new ClientModel());
            try
            {
        ...
        public async Task<List<AdminAuthorizeModel>> GetAdminClearance(string userId)
        {
            try
            {
                var clearanceLst = await _clearanceService.UserClearance(userId);
                if (clearanceLst == null)
                    return new List<AdminAuthorizeModel>();
                var clientIds = clearanceLst.Select(x => x.ClientKey).ToList();
                var query = context.Clients.Where(x => clientIds.Contains(x.Id));
                var res = await query.Select(x => new AdminAuthorizeModel { ClientKey = x.Id, ClientName = x.ClientName, ClientId = x.ClientId, }).ToListAsync();
